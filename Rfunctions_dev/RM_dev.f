C       use cudafor
      attributes(device) function RM_dev(m_N, c0, tau, taup, term, j_chi, coupling_Array)
      implicit none
      double precision :: RM_dev, m_N, c0, j_chi
...
c
      if (term.eq.c) then
         ...
         RM_dev = 1/c0**2 * (j_chi*(j_chi+1))/3. * c8*c8p
      else if (term.eq.q2) then
         RM_dev = 1/m_N**2 * (j_chi*(j_chi+1))/3. * c11*c11p
      else if (term.eq.v2q2) then
         RM_dev = 1/c0**2 * 1/m_N**2 * (j_chi*(j_chi+1))/3. * c5*c5p
      else
         RM_dev = 0.
      end if
c
